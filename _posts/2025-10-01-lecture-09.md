...
2. **Number of Trainable Parameters**
   - Each fully connected layer introduces weights and biases, and the total parameter count grows quickly with input size, hidden units, and output dimension.
   - Larger models capture complex patterns but risk overfitting.  
   - *Example:*  
     <img src="{{ '/assets/img/notes/lecture-09/slide10.png' | relative_url }}" width="500"/>

3. **Deep Learning Context**
   - Adding multiple layers results in *deep learning*.
   - Deeper networks can represent more complex functions but suffer from optimization challenges such as vanishing or exploding gradients.  
   - *Illustration:*  
     <img src="{{ '/assets/img/notes/lecture-09/slide11.png' | relative_url }}" width="500"/>

4. **Loss Landscape**
   - Unlike logistic regression or softmax regression (which have convex losses), the MLP loss surface is **non-convex**.
   - Training outcomes depend on initialization or data order, and multiple local minima are common.
   - Despite this, non-convex optimization works well in practice (e.g., LeCun’s perspective on non-convexity).  
   - *Illustration:*  
     <img src="{{ '/assets/img/notes/lecture-09/slide12.png' | relative_url }}" width="500"/>  
     <img src="{{ '/assets/img/notes/lecture-09/slide15.png' | relative_url }}" width="500"/>

5. **Initialization Effects**
   - If weights are initialized to all zeros, the network cannot learn because all neurons produce identical outputs.
   - Proper random initialization and normalization are essential for effective training.  
   - *Illustration:*  
     <img src="{{ '/assets/img/notes/lecture-09/slide18.png' | relative_url }}" width="500"/>
